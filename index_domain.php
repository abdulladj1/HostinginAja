...
<body>
	...
	<div class="container col-md-6 mt-4">
		<center><h1>Domain HostinginAja!</h1></center>
		<div class="card">
			<div class="card-header bg-success text-white ">
				Data Domain <a href="tambah_domain.php" class="btn btn-sm btn-primary float-right">ADD</a>
			</div>
			<div class="card-body">
				<table class="table table-bordered">
					...
					<tbody>
						<?php
							include('koneksi.php');
							$datas = mysqli_query($koneksi, "select * from domain") or die(mysqli_error($koneksi));
							$no = 1;
					...
					<tr>
						...
						<td>Rp <?= $row['harga_domain']; ?></td>
						<td><?= $row['desc_domain']; ?></td>
                        <td>
							<a href="foto/<?= $row['gambar_domain']; ?>" target="_blank">
							<img src="foto/<?= $row['gambar_domain']; ?>" style="width: 100px;">
							</a>
						</td>
						<td>
								<a href="edit_domain.php?id=<?= $row['id']; ?>" class="btn btn-sm btn-warning">Edit</a>
								<a href="hapus_domain.php?id=<?= $row['id']; ?>" class="btn btn-sm btn-danger" onclick="return confirm('anda yakin ingin hapus?');">Hapus</a>
						</td>
					</tr>
						<?php $no++; } ?>
					</tbody>
