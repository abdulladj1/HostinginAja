...
if (!isset($_SESSION['username'])) {
    header("Location: tambah_domain.php");
}
?>
<!DOCTYPE html>
<html>
			...
			<div class="card-body">
				<form enctype="multipart/form-data" action="" method="post" role="form">
					<div class="form-group">
						<label>Domain</label>
						<input type="text" name="nama_domain" required="" class="form-control">
					</div>
					<div class="form-group">
						<label>Harga</label>
						<input type="text" name="harga_domain" class="form-control">
					</div>
					<div class="form-group">
						<label>Deskripsi</label>
						<textarea class="form-control" name="desc_domain"></textarea>
					</div>
					<div class="form-group">
						<label>Gambar</label>
				...
				if (isset($_POST['submit'])) {
			        ...
			        $foto = $nama_unik1;	
					$datas = mysqli_query($koneksi, "insert into domain (nama_domain,harga_domain,desc_domain,gambar_domain)values('$nama', '$harga', '$deskripsi','$foto')") or die(mysqli_error($koneksi));
					echo "<script>alert('data berhasil disimpan.');window.location='index_domain.php';</script>";
				}
				?>
			</div>
		</div>
